...
class DeepFM2Model : public ModelZooImpl {
 ...
 protected:
  bool InitConfigKV(const std::string& k, const std::string& v) override {
    if (ModelZooImpl::InitConfigKV(k, v)) {
      ...
      if (!deepx_core::ParseDeepDims(v, &deep_dims_, k.c_str())) {
        return false;
      }
    } else {
 ...
 public:
  bool InitGraph(deepx_core::Graph* graph) const override {
    ...
    auto* Z1 = deepx_core::Concat("", {lin, quad3, deep});
    auto* Z2 = deepx_core::FullyConnect("Z2", Z1, 1);
    auto Z3 = deepx_core::BinaryClassificationTarget(Z2, has_w_);
