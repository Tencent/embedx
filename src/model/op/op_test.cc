...
class OpTestContext : public OpTestDataType {
 ...
 public:
  ...
  void CheckBackward(GraphNode* node, int on_heap,
                     ...
                     const param_initializer_t& post_param_initializer,
                     const inst_initializer_t& inst_initializer);
  void CheckOverwrittenParam(GraphNode* node, const std::string& name,
                             int on_heap,
...
void OpTestContext::CheckOverwrittenParam(
  ...
  InitClosedOpContext(inst_initializer);
  InitNumericalOpContext(inst_initializer);
  auto check = [this, name, &expected_overwritten_param]() {
    ClosedForward();
    ClosedBackward();
    auto* overwritten_param =
        closed_.overwritten_ptr().get<DataType::srm_t*>(name);
    EXPECT_SRM_NEAR_EPS(*overwritten_param, expected_overwritten_param,
                        CHECK_GRAD_EPS);
  };
...
void CheckOpOverwrittenParam(GraphNode* node, const std::string& name,
                             int on_heap,
                             const DataType::srm_t& expected_overwritten_param,
                             const param_initializer_t& pre_param_initializer,
                             const param_initializer_t& post_param_initializer,
                             const inst_initializer_t& inst_initializer) {
  OpTestContext op_test_context;
  op_test_context.CheckOverwrittenParam(
      node, name, on_heap, expected_overwritten_param, pre_param_initializer,
      post_param_initializer, inst_initializer);
}
